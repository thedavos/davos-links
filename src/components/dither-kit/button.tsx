...
export type DitherButtonProps = ComponentProps<"button"> & {
  ...
  color?: PixelColor
  /** Fill texture — chart variants plus the quieter local outline treatment. */
  variant?: ButtonVariant
  /** Glow on the dither fill. */
  bloom?: PixelBloom
}

export type DitherButtonSurfaceProps = {
  color?: PixelColor
  variant?: ButtonVariant
  bloom?: PixelBloom
}
...
function paintButton(
  ...
  const dotted = variant === "dotted" || variant === "dotted-subtle"
  const bias = variant === "dotted" ? 0.12 : variant === "dotted-subtle" ? 0.02 : 0
  for (let y = 0; y < rows; y++) {
    const density =
      variant === "gradient"
        ? 0.25 + 0.75 * ((y + 0.5) / rows)
        : dotted
          ? variant === "dotted-subtle"
            ? 0.32
            : 0.5
          : 0.75
 ...
 */
export function DitherButtonSurface({
  color = "blue",
  variant = "gradient",
  bloom = "off",
}: DitherButtonSurfaceProps) {
  ...
  useEffect(() => {
    ...
    const focus = () => setTarget(1)
    const blur = () => setTarget(hovered ? 1 : 0)
...
export function DitherButton({
  ...
  return (
    <button
      ...
      {...props}
    >
      <DitherButtonSurface color={color} variant={variant} bloom={bloom} />
